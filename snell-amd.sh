...
wget --no-check-certificate -O snell.zip https://github.com/surge-networks/snell/releases/download/v3.0.0b2/snell-server-v3.0.0b2-linux-amd64.zip
unzip -o snell.zip
rm -f snell.zip
...
if [ -f ${SYSTEMD} ]; then
  ...
  echo "StandardOutput=syslog" >>${SYSTEMD}
  echo "StandardError=syslog" >>${SYSTEMD}
  echo "SyslogIdentifier=snell-server" >>${SYSTEMD}
  echo "" >>${SYSTEMD}
